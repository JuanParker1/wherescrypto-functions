...
import { iGeniusUserResponse } from './models/igenius.model';
...
exports.onNewSignal = functions.firestore
	...
	.onWrite(async (change) => {
		const data = change.after.data() as Signal;
...
// iGenius Verify Code
const verifyClientRes$ = new BehaviorSubject<iGeniusUserResponse>({
	completion: 0,
	completionMsg: 'Sto ingranando',
});
const verifyClientInternalAsync = async (id: number): Promise<void> => {
	logger.info('verifyClientInternalAsync Started', 'verifyClientInternalAsync');
	await verifyClient(id, verifyClientRes$);
	logger.info('verifyClientInternalAsync Ended', 'verifyClientInternalAsync');
	verifyClientRes$.complete();
	return;
};
exports.verifyClient = functions.https.onCall((data) => {
	verifyClientInternalAsync(data.id);
