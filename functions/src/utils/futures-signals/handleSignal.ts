...
const getTakeProfit = (msg: string): SignalTarget => {
  const res: SignalTarget = {};
  const chars = "\n💎 take profit:";
  const a = msg.indexOf(`${chars}`);
  const b = msg.indexOf("\n🚨 stop loss");
...
const getStopLoss = (msg: string): SignalStopLoss => {
  const res: SignalStopLoss = {};
  const chars = "\n🚨 stop loss";
  const a = msg.indexOf(`${chars}`);
  const b = msg.indexOf("\n");
...
const getMargin = (msg: string): SignalMargin => {
  const res: SignalMargin = {};
  const chars = "\n💰 margine";
  const a = msg.indexOf(`${chars}`);
  const b = msg.indexOf("\n");
...
const getExchanges = (msg: string): SignalExchanges => {
  const m = msg.slice(msg.indexOf("exchange: "));
  const exchanges: SignalExchanges = [
    "Binance",
    "KuCoin",
    "Huobi",
    "UniSwap",
...
const getSetup = (msg: string): SignalSetup => {
...
export const handleSignal = (msg: string): Signal => {
